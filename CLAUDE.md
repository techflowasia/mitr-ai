...
- **UI page-data hooks (2026-06-10)**: `usePageData<T>(fetcher, deps, {errorMessage, enabled})` in `packages/ui/src/hooks/usePageData.ts` replaces the hand-rolled `isLoading+error+useEffect` fetch block (returns `{data, isLoading, error, refetch, setData}`, stale-response safe). `usePagination(initialSize)` in `usePagination.ts` for offset/limit state. Exemplars: CostsPage, ClawsPage. Migrate other pages as touched
- **UI modal primitives (2026-06-10)**: `MultiStepModal` in `packages/ui/src/components/MultiStepModal.tsx` owns backdrop/step-tabs/footer (Cancel/Back/Next/Submit) for step-based modals; `AgentFormSteps.tsx` has the shared info/model/tools steps + `useAgentFormData()`. CreateAgentModal/EditAgentModal are the exemplars; migrate other step modals (TriggerModal, ChannelSetupModal, ConfigureServiceModal…) as touched
